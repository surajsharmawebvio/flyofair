...
// Lazy load jQuery and other libraries only when needed
let $: any = null;

// Initialize jQuery and other libraries after app is mounted
const initializeLibraries = async () => {
    if (!$) {
        ...
        await import('./jquery-3.7.1.min.js');
        await import('./bootstrap');
        await import('./owl.carousel.2.3.4.min.js');

        // Initialize flatpickr globally
        const flatpickr = await import('https://cdn.jsdelivr.net/npm/flatpickr');
        (window as any).flatpickr = flatpickr.default;

        // Initialize UI components after libraries are loaded
        initializeUIComponents();
    }
};

// Initialize UI components that depend on jQuery and other libraries
const initializeUIComponents = () => {
    ...
    $(document).ready(function () {
        ...
        if ($(".date-input").length) {
            ...
            (window as any).flatpickr(".date-input", {
    ...
    // faq eyes
    (window as any).togglePassword = function(inputId: string, toggleIcon: HTMLElement) {
...
createInertiaApp({
    ...
    setup({ el, App, props, plugin }) {
        ...
        // Initialize libraries after app is mounted for better performance
        initializeLibraries();
...
export { $, initializeLibraries, initializeUIComponents };
